...
import { readDir, writeTextFile, exists, rename } from "@tauri-apps/plugin-fs";
...
export function useNotes(vaultPath: string | null) {
  ...
  const renameNote = useCallback(
    async (oldPath: string, newTitle: string): Promise<string | null> => {
      ...
      if (!cleanTitle) return oldPath; // no permitir título vacío
  ...
  return { notes, loading, reload: loadNotes, createNote, renameNote };
}
